...
| 2.18 | **Voice (Hindi/Hinglish)** — mic in Ask ShopMaster, mic in the customer search bar, **"bol ke listing"** (seller speaks → product form fills), browser TTS reads answers. Groq Whisper v3 turbo first, Gemini audio fallback | §3b A1–A2, decided 13 Sep | — |
| 2.19 | **Admin decision agent** — brief + recommended ruling for every open dispute and seller application, one-click approve/reject in /admin/orders and /admin/sellers; grounded in evidence + rulebook, human decides | §3b agents, decided 13 Sep | — |
| 2.20 | **Market insights** — Merchant API `price_competitiveness_product_view` + `best_sellers_product_cluster_view` → seller Grow page ("aapka daam market se X% upar", "is hafte category me top") + admin Google page | §3b C, decided 13 Sep | Google enables Market insights itself once the account has traffic (no button in Merchant Center Next; 13 Sep: 4 clicks/28 d, API says "not enabled"). Build the code, data arrives after cutover |
| 2.21 | **Similar products + Hinglish search** — product embeddings in the 3rd Atlas vector slot; "aapko ye bhi pasand aayega" on product page; search that maps jhumka⇄earrings⇄झुमका and survives typos | §3b A4, decided 13 Sep | — |
