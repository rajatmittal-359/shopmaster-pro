...
| G3 | Merchant Center API — per-product approved / disapproved + reason. **Migrated to Merchant API (products v1) 13 Sep** — Content API sunset 18 Aug 2026 | ✅ verified (18 approved) | **tick API developer on `rajatmittal6908` in People and access** (registerGcp done; waits on a verified API-developer user) | seller sees Google's verdict on the product ✅; admin sees all on `/admin/google` ✅ (17 approved, 35 not in the feed — other sellers' products; feed is Charming Jewels only, by design) | ✅ both |
...
| G6 | GA4 property + Data API — traffic, product views, add-to-cart funnel | ✅ | **create GA4 property → send Measurement ID (G-…) + Property ID; add the service account as Viewer; enable Analytics Data API** | ✅ built 13 Sep: gtag via `next/script`, page_view per route, `view_item / add_to_cart / begin_checkout / purchase / search`; `/admin/google/traffic` + Visitors card (sessions, funnel, top pages). Silent until `NEXT_PUBLIC_GA_MEASUREMENT_ID` / `GA4_PROPERTY_ID` are set | tag live 13 Sep (`G-SK8WB9BKFG`, Next + the React app the domain serves); Data API waits on Property ID + Viewer |
| G7 | PageSpeed Insights API — Core Web Vitals per page | ✅ | **enable PageSpeed Insights API; Create API key (restricted to it) → send** | ✅ built 13 Sep: `/admin/google/speed` (home, shop, a product page; mobile; daily), Speed card with score + LCP/CLS/INP(TBT) and Google's verdict words. Silent until `PAGESPEED_API_KEY` | code ✅ · Rajat ☐ |
| G8 | Google Business Profile — reviews, products, posts (local pack, "near me") | ✅ | **reviews 2→30, add products, one post a week** (Mummy, from `rajatmittal6908`); **send Instagram / Justdial / GBP Maps links** | ✅ built 13 Sep: `sameAs` on the Organization schema from `NEXT_PUBLIC_SAME_AS` (comma-separated) | code ✅ · Rajat ☐ |
| G9 | Merchant promotions feed — coupons shown in Shopping results | ✅ | — | coupons → promotions feed | ☐ later |
| G11 | **Google Customer Reviews** (Merchant Center programme) — after each order a one-question Google survey; enough answers earn the **seller-rating stars** under our name in Shopping and Search. Free; Flipkart/Myntra carry them | ✅ | Merchant Center → Growth → Manage programmes → Customer Reviews → enable; accept terms | opt-in snippet on the order-placed screen (order id, email, estimated delivery), the badge on the footer | ☐ |
...
| G13 | **Google Alerts** for "ShopMaster Pro", "Charming Jewels Jaipur" — who mentions us | ✅ | https://google.com/alerts, 2 minutes | — | ☐ |
