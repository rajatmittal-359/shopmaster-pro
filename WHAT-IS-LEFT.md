...
| 2.25 | **Agents, small** — seller growth note (weekly Hindi "3 kaam" email from Grow), catalogue agent (nightly: fix drafts, duplicates, alt text) · **MCP server** exposing the assistant's tools · ~~third text fallback~~ ✅ 14 Sep: **Cloudflare Workers AI road 4** (`utils/ai/cloudflareText`, Llama 3.3 70B with our tools through the gateway, 10k neurons/day shared with image edits; real tool call verified) · NVIDIA NIM reranker (later, if search quality asks) | §3b, decided 13 Sep | — |
