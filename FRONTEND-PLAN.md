...
### 4.32 The three navigations against the references (13 Sep 2026)

Rajat: the seller sidebar had become too thin; check admin and customer the same way. **References:** Meesho supplier panel, Flipkart Seller Hub, Amazon Seller Central's 2026 workspaces and Shopify admin for the seller; CS-Cart Multi-Vendor, Dokan and Mirakl operator for the admin; Amazon, Flipkart, Myntra and Meesho account menus and apps for the customer (table and sources in WHAT-IS-LEFT §3a).

- **Sidebar UI** (`PanelNav`, `PanelIdentity`, `PanelShell`): identity block on top (shop name / "Platform admin" — Shopify's store block), grouped headings, **count badges** fed by `/seller/nav-counts` and `/admin/nav-counts` every minute (Orders 2, Returns & issues 4 — Shopify/Meesho), sub-items under the active parent (Products → All · Photo studio · Stock history; the tab row now shows only on phones), Account group **pinned to the bottom** (Shopify's Settings), active item with a left accent bar plus the tint.
- **Seller — 9 items in 5 groups:** Home · Selling (Orders, **Returns & issues**) · Catalogue (Products, **Promotions**) · Money (Payments) · Account (**Performance**, Settings, **Help & rules**). New pages: `Issues.jsx` (returns / disputes with the 72-h clock / delivery problems), `Promotions.jsx` (seller-funded coupons, live at checkout, on /coupons and in the Google feed), `Performance.jsx` (Amazon Account Health: seven numbers against the rulebook, graded), `Help.jsx` (rules in one screen, eight questions, WhatsApp/email/phone).
- **Admin — two additions:** Catalogue → **Products** (every seller's catalogue with the listing score; Weak / Out of stock / Hidden), People → **Customers** (orders, spend, cancels, disputes, a stated flag rule, block/unblock with a reason).
- **Customer:** header = search · ♥ · bag · account (no "Shop"/"Contact" words); account menu in Flipkart's order (Orders, Saved, **Coupons**, **My reviews**, Addresses, **Account**, **Help**); new pages `/account` (name, password, **delete my account** — DPDP erasure, anonymised), `/reviews`, `/coupons` (public), `/help`; **bottom tab bar** on phones (Home · Shop · Saved · Bag · You) — the Flipkart/Meesho/Myntra app pattern.
- Backend: `controllers/panelController.js`, `utils/performance.js` (+ tests), `PATCH/DELETE /auth/me`, `POST /auth/change-password`, `GET /public/coupons`, `User.isBlocked` honoured by login and the auth middleware.

### 4.33 Shop filters (13 Sep 2026)

Rajat: the filter rail was a text list - "bakwaas tarika, mazaa nahi aa raha". **Reference:** Myntra's left rail and Flipkart's, Amazon's 2023 price slider, Baymard's filtering study. Built: folding sections with an on-count pill; checkbox-look rows that are still links (URLs stay shareable and crawlable); counts in quiet grey; **colour swatches** from a name→colour map (multicolour as a conic wheel), ring when selected; sizes as pill chips; **price as a two-thumb slider** (two native range inputs on one track, no library) with the typed boxes under it, applying on release/Enter; ratings as five stars; options with zero results not shown (dead-end rule). Applied chips are tinted with an × icon. **Phones:** a "Filters · N" button beside Sort opens the same panel in a left sheet (Myntra/Flipkart app); picking an option closes it.
- **Correction the same morning (Rajat):** category showed checkboxes but allowed one pick — a control lying about itself. Now: **category is a tree** (Amazon "Department" / Flipkart rail): inside a department only that branch shows, current in bold with an accent, "‹ All categories" above; the subcategory chip row above the grid stays on phones only. **Colour and size are real multi-selects** (`color=Gold,White`, server `$in`), rows with a checkbox + swatch; **rating is a radio** (one "at least N"). Every control's shape now says how many you may pick.
- **Dependent facets (13 Sep):** already the case — every facet is counted with all the other filters applied except its own, per category; zero options hide. Added the missing one: **rating counts** (`ratings: {4, 3}`) with the same rule, so "4★ & up" greys out with a 0 instead of leading to an empty page. Changing category clears refinements (previous note).

### 4.34 The category tree and who tends it (13 Sep 2026)

**Research:** Google Product Taxonomy (the 21 top levels Merchant Center classifies by), Flipkart / Meesho / Myntra / Amazon India department menus, and what Jaipur makes. **Result:** 23 main categories, 168 sub-categories (`backend/config/taxonomy.js`) - the Indian marketplace spine (Women's / Men's / Kids, Jewellery, Footwear, Bags, Watches, Beauty, Health, Electronics, Home & Kitchen, Furniture, Sports, Toys, Books, Grocery & Gourmet, Pets, Automotive, Garden, Musical Instruments) plus three the others do not lead with and Jaipur does: **Handicrafts & Art** (blue pottery, block prints, marble, lac, miniatures), **Puja & Festive**, **Gifts**. Every branch carries Google's own category path; the Merchant feed now sends `google_product_category` and a parent > leaf `product_type`. Storefront strips and tiles show only categories with products, so the wide tree costs nothing until it fills.

**Who manages:** the admin (create / rename / hide / Google path). A seller picks a leaf in the product form (type-to-search) and, if it is missing, files a request under the picker - name, main category, one line; the admin sees it on Categories with Create / Decline and the seller sees the answer in the same spot. Amazon, Flipkart and Meesho keep the taxonomy in the platform's hands for the same reason: two sellers naming one thing two ways breaks browsing and Google.
...
### 4.37 Settings - configuration out of the code (13 Sep 2026)

Rajat: "har chhoti cheez ke liye code me na aana pade" - and it is how every platform is built (Shopify Settings, WooCommerce Options, Magento Configuration): a settings document behind an admin page, the code holding the defaults, the hierarchy admin > seller > customer. Built: `models/PlatformSettings` (one document) with five blocks - **Who we are** (name, contact, address, hours, GSTIN), **Where else we are** (sameAs links), **Seller rulebook** (the eight numbers; saving a change **bumps the agreement version** and every seller accepts again - `config/sellerRules` is now a live object loaded from the document), **Switches** (COD, same-day, seller signup, free-delivery threshold, representative shipping rate - enforced by the API, not only hidden), **Announcement bar**. `/admin/settings` (pinned at the bottom of the admin nav), `GET /public/settings` for the storefront (5-min cache). Consumers so far: Footer, Contact, Help, the home page's trust block and Organization schema, the announcement bar above the header, checkout's COD option, the /sell page, the Merchant feed's shipping rate. Hindi toggle scoped to the seller panel only.

### 4.38 Ask ShopMaster - the assistant that learned the platform (13 Sep 2026)

Rajat: not a dictionary of canned answers - it must learn the architecture and the data itself; hard-coded text only as the floor. References: Shopify Sidekick (starter chips, answers that link to the page that does the thing), Amazon Seller Assistant (reads the seller's own orders and payouts before answering), Flipkart's help bot (Hinglish, short). Built as **retrieval + tools**, the pattern every one of those uses under the hood:

- **Knowledge index** - `backend/indexKnowledge.js` cuts the docs (CLAUDE, WHAT-IS-LEFT, FRONTEND-PLAN, project rules, DESIGN), the policy pages' prose, the seller panel's Help/Learn/Grow copy, the taxonomy, `config/knowledge.js`, and the *explanatory comments* of the backend and panel code into ~450 chunks; embeds them with Gemini `gemini-embedding-001` (768 dims, free); stores them in `KnowledgeChunk` with an **audience** wall (`everyone` / `seller` / `admin` - FRONTEND-PLAN and ops material are admin-only; `private/`, `.env`, credentials never indexed). Atlas Vector Search index `knowledge_vec` (the second of the three free slots); MongoDB text index on the same chunks is the fallback when a question cannot be embedded. Re-run after editing a doc or a comment: only changed chunks are re-embedded (hash), retags are free.
- **Tools** - `utils/ai/tools.js`: Gemini function calling over read-only lookups scoped by the session, never by the model's argument (`getOrder`, `myRecentOrders`, `myPayouts`, `productScore`, `myPerformance`, `platformSummary`, `findSeller`, `listCategories`, `searchKnowledge`, `webSearch` = a grounded Gemini call). A role only hears of its own tools. `utils/gemini.js#generateWithTools` runs the loop (≤6 rounds, model turn echoed whole for thought signatures).
- **Prefetch** - `utils/ai/assistant.js#contextFor`: the asker's most likely facts before the model runs; also the whole context when the fallback model (Pollinations nano, no tools) has to answer on a Gemini 429.
- **Floor** - `config/knowledge.js`: the platform in plain sentences with the live rulebook numbers and the page map per role.
- **Surfaces** - `/seller/ask` (nav, under Account), `/admin/ask` (chat + the 90-day log with each answer's lookups and passages, filter by role / not-helpful), customer on `/help` when signed in. `components/assist/{AskPanel,Answer,HelpAsk}`. Every answer shows *Checked: your payouts, the web…* - trust comes from seeing the source. Thumbs up/down → `AssistLog.helpful`. `aiLimiter` on the routes.
- **Measured** - seller payout question 16 s, dispute question 14 s, admin seller+platform 10 s (two tool calls). Verified live 13 Sep as the seller account, the customer test account and the admin.
