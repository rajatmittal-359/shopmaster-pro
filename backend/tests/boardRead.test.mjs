...
describe('readBoard', () => {
  it('returns the board text, whether it is a shop, and whether the name agrees', async () => {
    gemini.generate = vi.fn(async () => ({ ok: true, text: '{"text":"CHARMING JEWELS","isShop":true,"notes":"A jewellery shop front with a painted board."}' }));
    const r = await readBoard('https://res.cloudinary.com/x/shop.jpg', 'Charming Jewels');
    expect(r).toMatchObject({ ok: true, text: 'CHARMING JEWELS', isShop: true, matches: true });
  ...
  it('a stock photo or a different board is said plainly; model trouble is a dash, not an error', async () => {
    ...
    expect(await readBoard('https://x/y.jpg', 'Charming Jewels')).toMatchObject({ ok: true, text: '', isShop: false, matches: null });
    gemini.generate = vi.fn(async () => ({ ok: false, reason: 'quota' }));
    expect(await readBoard('https://x/y.jpg', 'Charming Jewels')).toMatchObject({ ok: false, reason: 'quota' });
    expect(await readBoard('', 'Charming Jewels')).toMatchObject({ ok: false });
  });
});
