...
- "Delivered but not received": the courier's proof of delivery (photo/signature) or OTP decides; orders at or above ₹${RULES.otpDeliveryAbove} are high-value: the courier's proof of delivery (photo/signature) is required before a "not received" claim is decided and an admin reviews it (Shiprocket has no doorstep-OTP switch; a courier's own OTP, where it exists, counts as proof - the admin switches it on at cutover). Without proof the benefit of the doubt is the customer's.
