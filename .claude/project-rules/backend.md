...
- **Notifications go through one door**: `utils/notify.js` `notify({userId, role, category, title, body, url, tag?, mail?})` writes the bell row (`models/Notification`, tag = dedupe), then push (`utils/push.js`, VAPID in env, `transport.send` is the seam) and mail as `User.notificationPrefs` allow; `notifyAdmins` for platform events. Event helpers: `utils/notifySeller.js`, `utils/notifyCustomer.js`. Never call `sendSafeEmail` for a person-facing event directly - pass it as `mail`. Categories: orders · returns · disputes · payouts · account · trust · growth · reminders (the bag reminder, `jobs/cartReminder`, daily from Actions - the only marketing-shaped mail; never add a coupon to it). Routes `/api/notifications` (any role). Tests: `bufferCommands` is off in `tests/setup.mjs` - an unstubbed model call rejects at once.
