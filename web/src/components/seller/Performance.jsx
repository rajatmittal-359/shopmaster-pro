...
const METRICS = [
  { key: 'cancelRate', title: 'Cancelled by you', fmt: (m) => (m.value == null ? '—' : `${m.value}%`), line: (m) => `Account review above ${m.threshold}%. 2 free a month, then ₹50 each.`, fix: 'Keep stock honest - an out-of-stock cancel is the commonest one.', href: '/seller/products' },
  { key: 'dispatchHours', title: 'Time to dispatch', fmt: (m) => (m.value == null ? '—' : m.value < 48 ? `${m.value} h` : `${Math.round(m.value / 24)} days`), line: (m) => `Rulebook: within ${m.threshold / 24} working days. Median of ${m.shipped} shipped.`, fix: 'Pack the same day; the courier is one button on the order.', href: '/seller/orders' },
  ...
  { key: 'rating', title: 'Rating', fmt: (m) => (m.value == null ? '—' : `${m.value} ★`), line: (m) => (m.count ? `${m.count} verified reviews.` : 'No reviews yet.'), fix: 'Photos that match the piece, and a note in the parcel asking for a review.', href: '/seller/products' },
  { key: 'listingQuality', title: 'Listing quality', fmt: (m) => (m.value == null ? '—' : `${m.value}/100`), line: (m) => `Average across ${m.products} live products. Google and shoppers read the same things.`, fix: 'Open a product - the score at the top says which three things to fix.', href: '/seller/products' },
];
    ...
    <div className="space-y-6">
      <div className={`rounded-xl border p-4 text-sm ${worst === 'review' ? 'border-destructive/40 bg-destructive/5' : worst === 'watch' ? 'border-amber-300 bg-amber-50 dark:bg-amber-950/30' : 'border-emerald-200 bg-emerald-50 dark:bg-emerald-950/30'}`}>
        <p className="font-medium">
          {worst === 'review' ? 'Something here crosses a rulebook line.' : worst === 'watch' ? 'Healthy, with a number or two to watch.' : 'Healthy. Nothing here needs you.'}
        </p>
        <p className="mt-0.5 text-muted-foreground">
          Last {data.days} days, {data.orders} orders. These are the numbers in the{' '}
          <Link href="/seller/help" className="text-brand-ink hover:underline">
            Seller Agreement
          </Link>
          ; a marketplace that keeps them is one buyers come back to.
        </p>
      </div>
        ...
        {METRICS.map((m) => {
          ...
          return (
            <PanelCard key={m.key} className="min-w-0">
              ...
              <p className="mt-1 text-xs text-muted-foreground">{m.line(v)}</p>
